...
last-modified: 2021-04-20T12:00:00.606Z
desc: |-
...
The root node is also a parent node in a way, but consider it the "Adam and Eve" of every other node and without it there would exit no other node. What one node considers a parent is a child to some other node. Like we have a root node which gives rise to every other node, there's also a leaf node which terminates the preceeding nodes along its path. Or have you seen a tree that grows a branch from a leaf?

My mother is a child to my grandmother and a parent to me. It's that complex&mdash;being a child and a parent at the same time. There also exits such relationship within the tree as we have in real life. Parent&ndash;to&ndash;child, child&ndash;to&ndash;child (or&nbsp;sibling).
...
What our original data would look like is this:
...
Events these tree node would observe and call handlers on are:
...
**A single stack for all child nodes**

We also maintain one single stack per node to occupy all the child nodes of that node if it's a branch node, otherwise the stack would be just empty. Without this we won't be able to deterministically know and access child nodes. The only way we'd be able to access child nodes without a stack of them would be to traverse the tree from bottom up, getting the parent of the least (base-est) node and checking every next and previous (sibling) node of that node to see they share a common parent. This would leave us a great deal of complexity.
